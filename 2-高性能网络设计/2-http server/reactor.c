...
#define CONNECTION_SIZE 1024 // 1048576
#define MAX_PORTS		20
...
int recv_cb(int fd)
{
    ...
    else if (count < 0) // 对方发送RST，手动关闭连接
    {
...
int send_cb(int fd)
{
	...
	if (conn_list[fd].status == 1) {
		printf("SEND: %s\n", conn_list[fd].w_buffer);
		count = send(fd, conn_list[fd].w_buffer, conn_list[fd].w_length, 0);
		set_event(fd, EPOLLOUT, 0);
	} else if (conn_list[fd].status == 2) {
        
        set_event(fd, EPOLLOUT, 0);
	} else if (conn_list[fd].status == 0) {
        if (conn_list[fd].w_length != 0) {
			count = send(fd, conn_list[fd].w_buffer, conn_list[fd].w_length, 0);
		}
